...
import { addYears, inRange, startOfMonthIso, startOfWeekIso, startOfYearIso } from "../utils/date";
...
function sumRevenue(inputs: DailyInput[], start: string, end: string): number {
  return inputs
    ...
    .reduce((acc, x) => acc + (Number(x.totalRevenue) || 0), 0);
}
...
export function computeKpis(all: DailyInput[], dateIso: string): KpiResult {
  ...
  const lastYearSameDayRevenue = Number(lastYearSameDay?.totalRevenue ?? 0);
  ...
  const last7 = all
    ...
    .slice(-7)
    .map((x) => ({ date: x.date, revenue: Number(x.woltRevenue ?? 0) }));
